...
class Template:
    ...
    def encode_path(
        ...
        for i, encoded_ids in enumerate(encoded_messages):
            message = messages[i]
            
            all_input_ids.extend(encoded_ids)
            
            if message["role"] == Role.USER:
                all_labels.extend([IGNORE_INDEX] * len(encoded_ids))
            else:
                all_labels.extend(encoded_ids)

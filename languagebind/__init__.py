...
class LanguageBind(nn.Module):
    def __init__(self, clip_type, use_temp=True, cache_dir='./'):
        ...
        for k, v in clip_type.items():
            ...
            else: # vllm和languagebind.from pretrained有冲突，默认走了modelscope，所以这里必须要用本地路径
                model = model_dict[k].from_pretrained(v)
